angular.module('movieModule').controller('RatingsController', function($scope, $rootScope, $routeParams, Ratings) {
    ...
    Ratings.show({user_id: $rootScope.uid, imdb_id: $scope.imdb_id}, function(data) {
    ...
    $scope.rateMovie = function() {
    	if ($scope.rated) {
    		Ratings.update({user_id: $rootScope.uid, imdb_id: $scope.imdb_id, rating: $scope.rate}, function(data) {
    			$scope.average = ($scope.average * $scope.votes + $scope.rate - $scope.old_rating) / $scope.votes;
    			$scope.old_rating = $scope.rate;
    		});
    	} else {
    		Ratings.create({user_id: $rootScope.uid, imdb_id: $scope.imdb_id, rating: $scope.rate}, function(data) {
